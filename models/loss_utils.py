...
def pts_det_loss(pc_score_inline, pc_score_outline, mask):
    det_loss = torch.mean(1-pc_score_inline) + torch.mean(pc_score_outline)
    return det_loss
...
def adapt_loss(img_features,pc_features,mask,margin=0.25, gamma=32):
    ...
    loss = loss_col.mean() + loss_row.mean()
